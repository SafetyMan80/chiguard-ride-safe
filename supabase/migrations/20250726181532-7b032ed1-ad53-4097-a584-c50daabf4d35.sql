...
-- 2. Implement server-side rate limiting using existing rate_limits table
-- This creates a robust rate limiting function that can be used across the application
...
BEGIN
  ...
  -- Get current request count for this endpoint in the time window
...
-- 3. Add server-side rate limiting to critical endpoints
-- Protect incident reporting endpoint
CREATE OR REPLACE FUNCTION public.rate_limit_incident_reports()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER SET search_path = ''
AS $function$
BEGIN
  -- Check rate limit: max 10 incident reports per hour per user
  IF NOT public.check_rate_limit('incident_reports', 10, 60) THEN
    RAISE EXCEPTION 'Rate limit exceeded. Too many incident reports. Please try again later.';
  END IF;
...
-- Add trigger for incident report rate limiting
DROP TRIGGER IF EXISTS rate_limit_incident_reports_trigger ON public.incident_reports;
CREATE TRIGGER rate_limit_incident_reports_trigger
  BEFORE INSERT ON public.incident_reports
  FOR EACH ROW EXECUTE FUNCTION public.rate_limit_incident_reports();

-- 4. Log this security migration
PERFORM public.log_security_audit(
  ...
  jsonb_build_object(
    ...
    'fixes_applied', jsonb_build_array(
      ...
      'server_side_rate_limiting_implemented',
      'incident_report_rate_limiting_added'
    )
  )
);